...
import { Upload, Heart, BrainCircuit, FileText, Activity, AlertCircle, User } from 'lucide-react';
...
const AIAnalysis = () => {
    ...
    const fetchPatients = async () => {
        try {
            ...
            // Fallback for demo if endpoint fails or doesn't exist yet
            // setPatients([{ id: 1, full_name: 'Demo Hasta', tc_no: '12345678901' }]);
        } finally {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex gap-4 border-b border-slate-200">
                <button
                    onClick={() => { setActiveTab('heart'); setResult(null); setError(null); }}
                ...
                >
                    <div className="flex items-center gap-2">
                        <Activity size={20} /> Kalp Hastalığı Riski (XGBoost)
                    </div>
                    {activeTab === 'heart' && (
                ...
                <button
                    onClick={() => { setActiveTab('xray'); setResult(null); setError(null); }}
                ...
                >
                    <div className="flex items-center gap-2">
                        <FileText size={20} /> Akciğer Röntgeni (DenseNet)
                    </div>
                    {activeTab === 'xray' && (
                        <div className="absolute bottom-0 left-0 w-full h-0.5 bg-primary rounded-t-full" />
                    )}
                            ...
                            {result ? (
                                <div className="space-y-6 animate-fade-in">
                                    ...
                                    <div className="p-4 bg-white rounded-xl border border-slate-200 shadow-sm">
                                        ...
                                        <div className={`text-lg font-bold ${result.prediction === 'Hasta' ? 'text-red-500' : 'text-green-500'}`}>
                                            {result.prediction}
                                        </div>
                                    ...
                                    <div className="p-6 bg-white rounded-xl shadow-sm border border-slate-100">
                                        ...
                                        <div className="flex items-center justify-between mb-6">
                                            <div>
                                                ...
                                                <span className={`text-2xl font-bold ${result.prediction.includes('Zatürre') ? 'text-red-500' : 'text-green-500'}`}>
                                                    {result.prediction}
                                                </span>
                                            </div>
                                        ...
                                        <div className="relative rounded-lg overflow-hidden bg-black aspect-video flex items-center justify-center">
                                            {uploadedImage && (
                                                <>
                                                    <img src={uploadedImage} alt="Original" className="absolute inset-0 w-full h-full object-contain opacity-50" />
                                                    {/* Real Heatmap Overlay will be implemented here */}
                                                </>
                                            )}
                                        </div>
                                        <p className="text-xs text-text-light mt-2 text-center">
                                            * Isı haritası, yapay zekanın odaklandığı bölgeleri temsil eder.
                                        </p>
